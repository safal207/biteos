...
В демо есть выбор между фастфудом и кафе Roby’s: покупатель открывает отдельное меню заведения, видит подходящие дополнения и оформляет демонстрационный заказ. Ресторан добавляет блюда и передаёт заказ, курьер забирает и отмечает доставку. Исходный киоск доступен через пункт «Киоск» слева или по ссылке с `#kiosk`.
