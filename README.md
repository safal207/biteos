...
**[Открыть онлайн-демо BiteOS](https://safal207.github.io/biteos/)** — меню, комбо, добавки, корзина и оформление демонстрационного заказа прямо в браузере.

Онлайн-демо на GitHub Pages работает без сервера. Каталог берётся из того же JSON, а расчёт и правила допродаж повторены в отдельном браузерном адаптере. Демозаказы существуют только в памяти вкладки до обновления страницы: оплата и передача в ресторан отсутствуют. Полный локальный вариант использует Go и Rust.
...
Статическая сборка находится в `apps/kiosk/dist-pages`, путь страницы — `/biteos/`. Обычный `pnpm build` продолжает собирать приложение для Go в `dist` и не включает демоадаптер. Если репозиторий переименован, обновите `base` в `vite.config.ts`.
